...
void setup(FILE **cfp, FILE **tfp, double *Ep, double *Virp, double *lp, gsl_rng **rg, unsigned long int *slcp, unsigned long int *sltp,
	...
	*vir6ijTrial     = (dVECTOR) malloc(numPairs*sizeof(double));
	...
	//fprintf(rhof,"Still still in! Rhof = %p\n",rhof);
	//fflush(stdout);
	fgrho();
	//fprintf(stdout,"Still still in! Rhof = %p\n",rhof);
	//fflush(stdout);
...
void qad() {
	...
	d = (rndm - 0.5)*2*maxStep;
	
//	#pragma omp single
	{
	rTrial[nm] = r[nm] + d;
	//fprintf(stdout,"Attempting to move particle %lu by %lf...",nm,d);
	}
...
//		#pragma omp single
		{
		dAcc[1]++;
		//printf("Wall collision. Move rejected...dRej = %lu\n",dAcc[1]);
		}
	}
...
//	#pragma omp single
	...
	//printf("E6Trial = %.5G...E12Trial = %.5G...dE = %.5G...",E6Trial,E12Trial,dE);
...
//		#pragma omp single
		{
		vAcc[0]++;
		//printf("Volume change accepted...dE = %.5G...vAcc = %lu\n",dE,vAcc[0]);
		E6  = E6Trial;
		E12 = E12Trial;
		E      = E12 - E6;
...
//		#pragma omp single
		{
		vAcc[1]++;
		//printf("Volume change rejected...dE = %.8G...vRej = %lu\n",dE,vAcc[1]);
		}
	}
}
